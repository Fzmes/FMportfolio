# Mon Portfolio - Data Science & IA - MESKINE FATIMA EZZAHRA

Bienvenue sur mon **portfolio personnel** ! 💻  
Découvrez mes projets en **Data Science, Machine Learning, IA** et visualisation de données.  


## 🛠️ Technologies Utilisées

<details>
<summary>Cliquer pour voir les technologies</summary>

| Technologie | Utilisation |
|------------|-------------|
| React.js ⚛️ | SPA dynamique, composants réutilisables |
| Styled-Components 🎨 | Thèmes et styles modernes |
| GitHub Pages 📦 | Déploiement du portfolio |
| JavaScript ES6+ 📝 | Logique et interactivité |
| HTML5 & CSS3 | Structure et style |
| Bootstrap 5 | Responsive et design rapide |

</details>

---

## Fonctionnalités Clés

- ⚡ **SPA rapide et responsive**  
- 🎯 **Navigation fluide** avec React Router (`basename` pour GitHub Pages)  
- 📊 **Projets interactifs** et visualisations de données  
- 💌 **Formulaire de contact fonctionnel**  
- 🌐 **SEO-friendly** et indexable par Google  
- 🖼️ **Animations modernes** pour une meilleure UX  

---

<p align="center">
  <img src="https://img.shields.io/badge/React-61DAFB?style=for-the-badge&logo=react&logoColor=white"/>
  <img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black"/>
  <img src="https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white"/>
  <img src="https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white"/>
</p>


---

## 💬 Contactez-moi

<p align="center">
  📧 Email : <a href="mailto:meskinefatimaezzahra@gmail.com">meskinefatimaezzahra@gmail.com</a><br/>
  💼 LinkedIn : <a href="https://www.linkedin.com/in/fatima-ezzahra-meskine">linkedin.com/in/tonprofil</a><br/>
</p>

---
<details>
<summary>Voir mes compétences techniques 🛠️</summary>

- React.js ⚛️  
- JavaScript ES6+  
- Styled-Components  
- Bootstrap 5  
- HTML5 & CSS3  

</details>

<details>
<summary>Voir mes hobbies 🎯</summary>
- Voyages ✈️
- Photographie 📸
- Cooking

</details>

---

## 🎉 Merci de visiter mon portfolio !  

N’hésitez pas à explorer mes projets et à me contacter pour collaborer !  
